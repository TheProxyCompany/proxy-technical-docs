...
The Life Map is Eden's knowledge graph — your life, mapped as a graph — a structured representation of everything you're working on, tracking, and thinking about. It's stored in Trellis, a graph database built on sled (an embedded key-value store).
...
| **Entity** | A thing, person, or object | — | "PIE", "Eden", a person's name |
...
| **WANTS** | Aspirational pull (softer than NEEDS) | — | "eden WANTS semantic-search" |
...
When Eden is running, the Life Map is queryable via MCP tools on `localhost:51711`:

```bash
# List all actions in progress
eden_list entity_type=action filters={"phase": "doing"}

# Get a specific node
eden_get entity_type=action id="ship-v1"

# See the structural overview
eden_overview limit=10 depth=2

# Recent activity
eden_recent limit=20

# Add an edge
eden_add_edge from_id="a" to_id="b" edge_type="NEEDS"
```
